...
# 子任务5（紧急）：重新生成全部 description（≥150 中文字符）✅
- 背景：问题.md 更新需求——所有文章 description 需重新生成，不少于 150 个中文字符
- 方案：逐篇阅读正文 → 手工撰写描述 → 脚本统一写入 frontmatter（yaml 解析保留其他字段，lineWidth 0 单行输出）
- 注意事项：6 篇根目录文章（彪马野郎/空之境界/汕头之旅/深圳打工记/梧桐山/装机日志）在 git 历史中本就没有正文（只有 frontmatter），描述基于标题/标签/原描述撰写
## 用户选择的处理方案
- 人工撰写 + 脚本批量写入
- 最终处理结果：53 篇全部更新，平均 203 汉字，最低 ≥150；pnpm build 153 页零报错；详情页 meta 与列表页卡片渲染验证通过

# 附：发现的 git 状态问题 ✅
- 根目录 11 篇拼音文件在工作区消失（git 显示 staged 删除），但 HEAD（026781a）中有完整内容
- 处理：git checkout HEAD -- 恢复 11 篇，工作区恢复干净
