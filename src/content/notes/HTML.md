...
除了上面这些属性，全局属性还包括事件处理属性（event handler），用来响应用户的动作。这些属性的值都是 JavaScript 代码。
...
###### `标签。多个` `可以放置在同一个` `里面，一个` `里面也可能包含多个` `，这取决于` `和` `在当前页面的含义。 `<nav>``<nav>`标签用于放置页面或文档的导航信息。

1. [商品 A](item-a)
2. [商品 B](item-b)
3. 商品 C

###### 一般来说，`<nav>`往往放置在`<header>`里面，不适合放入`<footer>`。另外，一个页面可以有多个`<nav>`，比如一个用于站点导航，另一个用于文章导航。 `<nav>`里面通常是列表，但也可以放置其他标签。 `<h1>` ~ `<h6>`HTML 提供了6个标签，用来表示文章的标题。按照标题的等级，一共分成六级。 - `
...
`<address>`标签是一个块级标签，表示某人或某个组织的联系方式。
