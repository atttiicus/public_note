...
\text{产生式} & \text{语义动作} \\
\hline
E \to E*1 + T &\{ E.\text{node} = \text{new Node (“+”}, E₁.\text{node}, T.\text{node}); \} \\
E \to T &\{ E.\text{node} = T.\text{node}; \} \\
T \to T_1 * F &\{ T.\text{node} = \text{new Node (“\_”}, T₁.\text{node}, F.\text{node}); \} \\
...
\text{产生式} & \text{语义规则} \\
\hline
S \to \text{if} (B) \ M*1 \ S_1 \ N \ \text{else} \ M_2 \ S_2 & \text{backpatch}(B.\text{truelist}, M_1.\text{instr}); \\
...
& \text{emit}(\text{“goto”\_\_\_\_}); /* 稍后回填 \_/ \\
\hline
M \to \varepsilon & M.\text{instr} = \text{nextinstr}; \\
\hline
