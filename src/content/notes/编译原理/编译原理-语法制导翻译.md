---
title: 编译原理-语法制导翻译
description: 语法制导定义（SDD）中 S 属性与 L 属性的分析、依赖图的构造与求值顺序，以及在产生式体中嵌入语义动作（SDT）并生成抽象语法树（AST）的完整方法。
tags: ['计算机科学', '编译原理']
series: 编译原理
order: 4
---
...
\hline
T' \rightarrow _FT_1'
& T_1'.\text{inh} = T'.\text{inh} _ F.\text{val} \\
...
- 继承属性，且对于任意产生式 $ A \rightarrow X_1 X_2 ... X_n \in P $，$ X_j $ 的继承属性仅依赖于：
    - 产生式中 $ X*j $ **左边** 符号 $ X_1, X_2, ..., X*{j-1} $ 的属性
- $ A $ 的继承属性
...
$ \text{T} \rightarrow \text{T}\_1 _ \text{F} \{\text{T.val} = \text{T}\_1.\text{val} _ \text{F.val}\} $
...
3. & E \rightarrow T \\
4. & T \rightarrow \{ \text{print}(\text{“_”}); \} \; T_1 _ F \\
5. & T \rightarrow F \\
6. & F \rightarrow (E) \\
...
\quad \quad C(\text{next, L2}); \quad /_ 当条件为真时跳转为 S 最开始的 next 标号，否则跳转至 L2 _/ \\
