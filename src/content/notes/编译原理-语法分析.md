...
1. 如果 $ X $ 是非终结符，并且 $ X \rightarrow Y_1Y_2\cdots Y_k $ 是一个产生式：
    - 如果某个 $ a $ 在 $ \text{First}(Y_i) $ 中，并且 $ \varepsilon $ 在 $ \text{First}(Y_1), \text{First}(Y_2), \cdots, \text{First}(Y_{i-1}) $ 中，那么 $ a $ 也在 $ \text{First}(X) $ 中  **人话**：如果 $ \varepsilon $ 在这些 $ \text{First}(Y_i) $ 中，那么就意味着 $ Y_i \Rightarrow^* \varepsilon $，也就可以忽略前面的
