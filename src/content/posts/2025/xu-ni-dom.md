---
title: 虚拟DOM
description: 虚拟 DOM 是当下非常火的技术，两大著名前端框架 React 和 Vue 都在使用。文章结合浏览器工作机制对虚拟 DOM 进行分析：先讲 DOM 的缺陷——通过 JavaScript 操作 DOM 会影响整个渲染流水线，频繁的节点修改会反复触发重排与重绘，性能开销巨大；在此基础上介绍虚拟 DOM 如何解决这些缺陷——用 JavaScript 对象描述真实的 DOM 结构，通过 diff 算法比较前后两棵虚拟树的差异，把多次操作合并为一次精准的批量更新。文章最后还站在双缓存与 MVC 的视角审视虚拟 DOM：双缓存让中间态不直接上屏，MVC 把渲染逻辑与状态管理解耦。理解这些，能对前端框架有更底层的认识。
pubDatetime: 2025-01-08 12:00:00
tags:
    - 前端
    - 性能
---
