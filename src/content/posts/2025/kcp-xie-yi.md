---
title: KCP 协议
description: KCP 是一种基于 UDP 的快速可靠传输协议：官方描述中，它可以用比 TCP 浪费 10%-20% 带宽的代价，换取平均延迟降低 30%-40%、最大延迟降低三倍的效果。KCP 是纯算法实现，不负责底层数据包收发，需要使用者自行定义下层传输方式，连时钟都需要外部传入，内部没有任何系统调用；整个协议只有 ikcp.h 与 ikcp.c 两个源文件，可方便地集成进自己的协议栈。简单来说，KCP 就是在 UDP 之上再做一层封装，实现 TCP 的效果并弥补 TCP 的不足。文章详细解析了 KCP 的帧头结构：8 字节对齐、空包 24 字节，逐字段讲解 conv 会话标识、cmd 命令、frg 分片、wnd 窗口等含义。
pubDatetime: 2025-05-24 15:30:00
tags:
    - 网络
---
