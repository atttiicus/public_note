---
title: 网页多图优化
description: 根据 httparchive.org 的数据，图像是当下网站中占用最多带宽的资源类型，图片加载过慢对用户体验的影响非常明显。文章明确区分了两类优化思路：图片本身的优化——压缩或选择更合适的图片格式，以及网络传输的优化——用 CDN 加速，后者多数情况要花钱解决。本文重点探讨图片加载策略，即「懒加载」：推迟加载非视口内的图片，从而减少资源请求数量。文章介绍了三种主流懒加载方案——img 标签的 loading 属性（Chrome 76+ 原生支持）、Intersection Observer API、以及基于 scroll/resize 等事件的监听方案，逐一说明各自的使用方式、兼容性与适用场景，帮助开发者用最合适的方式提升多图页面的加载性能。
pubDatetime: 2025-01-24 20:30:00
tags:
    - 前端
    - Web开发
---
