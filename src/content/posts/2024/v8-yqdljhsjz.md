---
title: V8 引擎的垃圾回收机制
description: 使用过的数据不再需要时就成了垃圾数据，如果一直占用内存，内存会越用越少，所以必须回收。文章先对比两种回收策略：C/C++ 的手动回收——何时分配、何时销毁都由代码控制，忘记 free 就会造成内存泄漏；以及 JavaScript、Java、Python 等的自动回收——垃圾数据由垃圾回收器释放。随后深入 V8 的实现：新生代内存使用 Scavenge 算法（半空间复制、对象晋升），老生代使用标记-清除与标记-整理算法，以及增量标记、并行回收等降低主线程停顿的优化手段。同时解释了栈空间与堆空间在回收上的区别。理解垃圾回收机制，是写出高性能、无泄漏 JavaScript 代码的重要基础。
pubDatetime: 2024-05-31 13:30:00
tags:
    - 前端
    - JavaScript
    - V8引擎
