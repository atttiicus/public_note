...
description: JavaScript 的内存机制是不常被提及、因此容易被忽视的概念，但要成为专家并打造高性能前端应用，就必须要搞清楚它。文章用两段看似简单的代码引出问题：同样的赋值操作，为什么打印出的结果不同？由此揭示 JavaScript 数据的存储方式——基本类型（原始值）存放在栈中，复制时按值拷贝；引用类型（对象、数组、函数）的引用地址存在栈中、真正的数据存放在堆中，复制时共享同一份堆数据。理解了栈与堆的分工，才能解释为何修改一个变量会影响另一个变量，也才能避免闭包、浅拷贝等场景中常见的坑。本文是内存机制系列的第一篇。
