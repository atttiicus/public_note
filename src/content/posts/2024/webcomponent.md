---
title: webComponent
description: 组件化开发是程序员的天然需求，其精髓用十个字概括就是「对内高内聚，对外低耦合」：组件内部元素彼此紧密结合，对外与其它组件的联系最少、接口简单。文章先讲组件化对多人协作项目的价值——降低系统耦合度、降低沟通复杂度、让系统易于维护——并对比 C/C++、Java 等语言在语言层面对组件化的支持。而 JavaScript 作为一门脚本语言，天然缺乏模块作用域等机制，Web Components 正是浏览器层面给出的解决方案：Custom Elements 自定义元素、Shadow DOM 隔离样式与结构、HTML Templates 模板复用。文章系统介绍了这套原生组件标准的工作原理与使用方式，让组件化能力摆脱框架的依赖。
pubDatetime: 2024-10-06 13:30:00
tags:
    - 前端
    - HTML
---
