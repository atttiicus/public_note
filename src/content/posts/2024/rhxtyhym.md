---
title: 如何系统优化页面
description: 页面优化本质上就是让页面更快地显示和响应，而一个页面在生命周期不同阶段的关注点完全不同。文章把页面划分为三个阶段：加载阶段（从发出请求到渲染出完整页面，主要受网络与 JavaScript 脚本影响）、交互阶段（主要受 JavaScript 执行影响）、关闭阶段（清理操作），并重点分析前两个阶段。针对加载阶段，文章结合渲染流水线讨论哪些资源会阻塞首次绘制、如何减少关键资源的下载与解析时间；针对交互阶段，则讨论如何减少长任务、缩短每个任务的执行时间，避免阻塞主线程。文章给出了一套系统化的优化思路，帮助开发者从全局视角而非零散技巧来提升页面性能。
pubDatetime: 2024-12-30 22:30:00
tags:
    - 前端
    - 性能
---
