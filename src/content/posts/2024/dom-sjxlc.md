---
title: DOM 树解析流程
description: 在渲染流水线中，几乎所有后续步骤都直接或间接依赖 DOM 结构，本文沿着网络数据流介绍 DOM 树是如何生成的。首先明确 DOM 的三层作用：它是生成页面的基础数据结构、提供给 JavaScript 操作页面的接口、也是一道安全防护线——不安全的内容在解析阶段就被拦截。随后剖析 HTML 字节流解析为 DOM 的完整过程：字节流如何分词、标记化、构建节点树，以及遇到 JavaScript 脚本时解析器如何暂停等待脚本执行、内联脚本与外链脚本对解析的不同影响，还有如何处理跨站点资源。对理解浏览器渲染原理、优化页面首屏性能很有帮助。
pubDatetime: 2024-12-08 13:30:00
tags:
    - 前端
    - 性能
    - HTML
