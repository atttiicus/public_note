...
description: 当 JavaScript 代码被执行时，引擎会先编译并创建执行上下文——执行全局代码会创建全局执行上下文（整个页面生命周期只有一份），调用函数会创建函数执行上下文，使用 eval 也会创建执行上下文。文章在理解执行上下文的基础上深入调用栈：调用栈如何用「后进先出」的方式管理函数的嵌套调用，栈顶是正在执行的任务，函数返回后其执行上下文被弹出销毁。同时解释了栈溢出（Stack Overflow）的产生原因——递归调用过深导致栈空间耗尽，并展示了浏览器报错信息如何帮助我们定位问题。掌握调用栈，既能理解引擎工作原理、提升调试能力，也是面试中的高频考点。
pubDatetime: 2024-03-15 13:30:00
tags:
    - 前端
    - JavaScript
    - 浏览器
