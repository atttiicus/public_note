---
title: 使用安卓平板or手机写代码
description: 作者有短期出门的需求，笔记本电脑又借给了朋友，但又想随时写博客、写笔记并直接同步到 GitHub——手机和平板自带的笔记软件用不顺手，于是萌生了在安卓设备上完成基本编码的想法。文章是作者的「捣鼓手记」而非教程：需要准备一台安卓 7.0 以上的平板（手机屏幕太小不建议）、代理软件（如 Clash、V2ray）、termux、git 与 code-server。操作步骤从安装 termux 开始——官网下载需要代理访问——接着更新 termux 环境（否则后续安装 code-server 会出现依赖错误），再逐步配置 code-server 与 git 同步。记录了整个过程中的踩坑与解决方式，适合想尝试移动端开发环境的读者参考。
