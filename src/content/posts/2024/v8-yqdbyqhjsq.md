---
title: V8 引擎的编译器和解释器
description: 前端工具与框架更新飞快，要追上节奏就需要抓住本质知识——理解 V8 如何执行 JavaScript，能帮助深入理解 Babel、ESLint、Vue、React 等工具的底层实现。文章从编译器与解释器的基本概念讲起：机器无法直接理解代码，需要翻译成机器语言，按执行流程可把语言划分为编译型与解释型两类。随后深入 V8 的执行机制：JavaScript 代码如何被解析为抽象语法树（AST）、如何生成字节码、解释器如何逐条执行字节码、热点代码如何被编译器优化为机器码（JIT 即时编译），以及内联缓存等优化技术。读完会对「V8 如何执行一段 JavaScript 代码」有完整的认识。
pubDatetime: 2024-06-05 13:30:00
tags:
    - 前端
    - JavaScript
    - V8引擎
