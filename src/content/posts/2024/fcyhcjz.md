---
title: 分层与合成机制
description: 分层与合成机制代表了浏览器最先进的合成技术，理解它有助于更深刻地理解 CSS 动画与 JavaScript 的底层工作机制。文章从显示器如何显示图像讲起：显示器以固定频率（通常 60Hz）读取显卡前缓冲区中的图像，显卡负责合成新图像并交换前后缓冲区。由此引入帧与帧率的概念，再深入 Chrome 的分层架构：页面元素如何根据属性被拆分成独立的图层（图层树），哪些情况会触发元素独立成层（transform、opacity、will-change 等），以及合成线程如何只更新变化的图层、把合成操作交给 GPU 完成。这解释了为什么用 transform 做动画比修改布局属性更流畅——只影响合成，不触发重排重绘。
pubDatetime: 2024-12-22 13:30:00
tags:
    - 前端
    - 性能
    - CSS
