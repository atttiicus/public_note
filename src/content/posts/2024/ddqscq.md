---
title: 迭代器 & 生成器
description: 迭代的英文「iteration」源自拉丁文 itero，意思是重复或再来。ECMAScript 6 新增了迭代器与生成器两大特性，目的就是让开发者能够更清晰、高效、方便地实现迭代。文章从计数循环这一最基础的迭代形式讲起，逐步展开：迭代器模式如何让集合遍历与实现解耦——调用者无须知道数据结构的内部实现；可迭代协议与迭代器协议的区别；生成器如何用 yield 关键字暂停与恢复执行，从而以同步代码的写法表达异步与惰性求值的逻辑。文中以 JavaScript 为例配以大量代码演示，并结合与其他语言迭代器实现的对比，帮助读者彻底掌握这两个进阶特性。
pubDatetime: 2024-02-20 12:00:00
tags:
    - OOP
    - JavaScript
---
