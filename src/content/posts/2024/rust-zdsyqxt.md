---
title: Rust 中的所有权系统
description: 作者在空闲时间研读《Rust 程序设计语言》，对 Rust 的「所有权」系统产生了浓厚的兴趣。文章先交代背景：由于后续要研究 WebAssembly 与 Tauri，两者都基于 Rust，所以决定提前预习这门语言。所有权（ownership）是 Rust 最核心的特性，解决的是所有程序都要面对的内存管理问题——C/C++ 要求开发者手动分配释放内存，Java/C# 依赖内置的垃圾回收（GC），而 Rust 独创性地通过所有权规则在编译期保证内存安全：每个值有且只有一个所有者、所有权可以转移、借用与引用的生命周期规则，既没有手动管理的负担，也免去 GC 的运行时开销。文章讲解清晰，配以大量代码示例，非常适合 Rust 初学者。
pubDatetime: 2024-10-10 12:00:00
tags:
    - Rust
---
