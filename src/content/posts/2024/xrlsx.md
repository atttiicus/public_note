---
title: 渲染流水线
description: CSS 决定了页面最终显示效果与用户的第一体验，搞清楚浏览器中的 CSS 是怎么工作的，才能理解如何优化页面。文章站在渲染流水线视角讲解 CSS 的工作方式：网络进程接收 HTML 后交给渲染进程，渲染进程解析 HTML 构建 DOM 树、解析 CSS 构建 CSSOM，接着将两者合并生成布局树，经过布局计算、绘制与合成最终呈现在屏幕上。文章特别指出请求 HTML 与构建 DOM 之间的空闲时间可能成为渲染瓶颈，并重点分析 CSS 对首次加载白屏时间的影响：渲染树需要等 CSS 与 JS 就绪，外部样式表会阻塞渲染。最后给出优化建议——尽早加载关键 CSS、避免渲染阻塞，从而缩短白屏时间、提升首屏体验。
