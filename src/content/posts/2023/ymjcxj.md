---
title: 页面进程详解
description: 打开一个页面，Chrome 任务管理器里却显示启动了 4 个进程，这是为什么？文章从进程与线程的概念讲起，通过并行处理示例帮助读者区分二者的关系，然后层层展开浏览器多进程架构的由来：为什么 Chrome 要把渲染进程放进安全沙箱——因为渲染进程执行的代码来自网络、不可信任，多进程隔离可以把恶意攻击的影响限制在单个进程内。文章随后详细梳理了浏览器进程、渲染进程、网络进程、GPU 进程各自的分工，以及同一站点（同一域名）页面如何共享渲染进程的优化策略，让读者彻底理解现代浏览器稳定、安全又高效背后的架构设计。
pubDatetime: 2023-10-12 20:30:00
tags:
    - 前端
    - 浏览器
    - 计算机科学
