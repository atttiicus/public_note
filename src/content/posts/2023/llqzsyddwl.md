---
title: 浏览器中使用到的网络
description: FP（First Paint，首次绘制时间）是衡量网页性能的重要指标，直接影响用户跳出率，而影响 FP 的关键因素之一就是网络加载速度。要优化网页加载，就必须理解 TCP/IP 协议体系：文件在网络中被拆分为数据包传输，数据包又有丢失或出错的可能。文章站在数据包的视角回答「如何保证页面文件被完整送达浏览器」：从数据包如何通过 IP 寻址送达目标主机、ARP 协议如何解析 MAC 地址、数据包如何被网卡与操作系统转交给应用进程，再到 TCP 如何通过三次握手、确认应答、超时重传与流量控制保证可靠传输，把网络原理讲得清晰透彻。
pubDatetime: 2023-10-16 20:30:00
tags:
    - 前端
    - 浏览器
    - 网络
