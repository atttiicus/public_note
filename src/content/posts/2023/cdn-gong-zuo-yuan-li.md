---
title: CDN 工作原理
description: CDN 即内容分发网络，允许将静态资源分发到多个不同的机房，实现用户的就近访问，从而加快静态资源的访问速度、减轻服务器与带宽的负担。文章把「内容分发网络」拆开解读：内容是图片、视频、文档、JS、CSS、HTML 等静态资源；分发网络则是把这些资源分发到位于不同地理位置的机房服务器上，比如北京的用户直接访问北京机房的数据。作者用京东的仓储物流体系做类比，帮助理解 CDN 作为服务上一层特殊缓存服务的定位。随后进入工作原理：讲解预热方式如何将源站资源同步到 CDN 节点、回源机制的触发条件，以及回源导致响应变慢的原因，是一篇通俗易懂的 CDN 入门文章。
pubDatetime: 2023-03-31 12:00:00
tags:
    - 网络
---
