---
title: HTTP 长连接与短链接
description: 面试时被问到 HTTP 长连接与短连接的问题，当时没答好，作者回来补课写下这篇记录。文章首先明确核心概念：HTTP 短连接下浏览器和服务器每进行一次 HTTP 操作就建立一次连接，任务结束即断开；长连接则复用同一个 TCP 连接承载多次请求，省去反复建立与断开连接的消耗。接着梳理了 HTTP/1.0 默认短连接、HTTP/1.1 起默认长连接的历史演变，并说明通过请求头与响应头中的 Connection 字段设置为 keep-alive 来启用长连接的方法。最后讨论了长连接超时等实际使用中的注意事项，是一篇简洁实用的网络基础补充笔记。
