---
title: 字符串中文识别
description: 汉字是表意文字，字符数量庞大，字体与排版都比英文复杂，不少计算机系统无法完整展示中文字符；而在国内开发中，限制用户输入中文（中文昵称、中文密码、聊天内容等）又是非常常见的需求。文章以 JavaScript 为重点，介绍识别中文的几种正则写法：适用于大多数场景的常用正则、简单粗暴的汉字区间匹配（但会误伤逗号句号等符号）、以及按 Unicode 中文字符对照表编写的更精准的正则。同时也提到中文总数超过十万，日常交流所需的常用汉字不过三万字，对生僻字的处理需要额外考虑。对其他语言如 Java、Golang 的中文识别也有参考意义。
pubDatetime: 2023-03-22 12:00:00
tags:
    - JavaScript
    - Java
    - Golang
