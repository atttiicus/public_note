---
title: 浏览器跨 Tab 窗口通信
description: 浏览器中不同窗口（包括不同标签页、不同浏览器窗口甚至不同浏览器实例）之间如何互相通信？纯前端方案下，无法借助 WebSocket 与后端转发，本文系统梳理了跨 Tab 通信的实现方式。文章以通信需要的数据传输能力、实时性与安全性为前提，重点讲解 Broadcast Channel 这一基于发布-订阅模式的 Web API：在一个窗口创建 BroadcastChannel 对象即可向其他窗口广播消息，同时覆盖了 localStorage 事件监听、postMessage 等常用替代方案的使用场景与局限。对需要实现多窗口状态同步、登录态联动等功能的前端开发者很有参考价值。
pubDatetime: 2023-06-18 12:00:00
tags:
    - 前端
    - JavaScript
    - Vue
