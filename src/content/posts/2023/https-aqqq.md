---
title: HTTPS 安全请求
description: HTTP 从设计之初就只考虑传输超文本文件，数据以明文在网络中传输，这意味着用户电脑、WiFi 路由器、运营商、目标服务器之间的任何一环都可能窃取或篡改数据，这就是典型的中间人攻击。鉴于明文传输制约了网购、在线转账等场景，HTTPS 在 TCP 与 HTTP 之间插入了一层安全层来加密数据。文章详细拆解了 HTTPS 的加密方案：通过对称加密保证传输效率、用非对称加密解决密钥分发问题，再借助 CA 数字证书来验证服务器身份、防止伪造，从而构建起既安全又高效的通信通道，并解释了证书链与信任体系的工作原理。
pubDatetime: 2023-10-22 20:30:00
tags:
    - 前端
    - 浏览器
    - 网络
...
之前提到，有了数字证书，黑客就无法欺骗用户了，不过我们并没有解释清楚如何通过数字证书来证明用户身份，所以接下来我们再来把这个问题解释清楚。
