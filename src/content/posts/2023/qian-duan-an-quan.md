---
title: 前端安全
description: 前端安全的目标是让 Web 应用在客户端免受攻击与数据泄露，需要从输入输出处理、依赖管理、数据存储与传输多个层面防护。文章开头就点出最核心的一条铁律：不要在页面中插入任何不可信数据，除非已按编码规则处理。随后重点剖析三大攻击类型——XSS 跨站脚本攻击：攻击者向页面注入恶意脚本，防范秘诀是对输入做验证、对输出做编码；CSRF 跨站请求伪造：利用用户已登录的浏览器发送伪造请求，需要从同源检测与 Token 校验入手；MITM 中间人攻击：通过 HTTPS 加密通信来抵御。同时也提醒读者，安全需要高成本投入，与敏捷开发之间存在权衡。
pubDatetime: 2023-04-14 12:00:00
tags:
    - 前端
    - Web开发
    - 安全
